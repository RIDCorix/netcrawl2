...
layerRoutes.get('/layers', (req: Request, res: Response) => {
  ...
  const layers = LAYER_DEFS.map(def => {
    ...
    if (thresh) {
      if (thresh.total_data_deposited !== undefined)
        progress.total_data_deposited = getStat('total_data_deposited', uid);
...
layerRoutes.post('/layer/switch', (req: Request, res: Response) => {
  ...
  const activeWorkers = getWorkers(uid).filter((w: any) => ['running', 'moving', 'harvesting'].includes(w.status));
