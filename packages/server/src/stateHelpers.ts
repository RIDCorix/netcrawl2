 ...
 */
export function checkCost(resources: Resources, cost: Record<string, number>): string | null {
 ...
 */
export function deductCost(resources: Resources, cost: Record<string, number>): Resources {
 ...
 */
export function updateNodeData(nodes: GameNode[], nodeId: string, updater: (data: NodeData) => NodeData): GameNode[] {
  return nodes.map(n => (n.id === nodeId ? { ...n, data: updater({ ...n.data }) } : n));
}
