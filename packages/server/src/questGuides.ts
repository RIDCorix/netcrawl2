...
3. Search "Python" → Install the Microsoft Python extension` },
    ...
    { title: 'Set Up Your Workspace', content: `**Step 1:** Install **uv** (Python package manager):
\`\`\`powershell
...
- \`workers/\` — your worker classes go here` },
    ...
    { title: 'Configure main.py', content: `Open \`workspace/workspace_example/main.py\` in your editor. Find the \`NetCrawl(...)\` section and update the **server URL**.
...
\`\`\`` },
    ...
    { title: 'Run Your Code Server', content: `Now start your Python code server. In the \`workspace/workspace_example/\` folder:
