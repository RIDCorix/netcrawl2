  ...
  q_hello_world: [
    {
      ...
      content: `Every worker has two methods:
...
The game runner calls \`on_startup()\` once, then keeps calling \`on_loop()\` until you stop the worker.`,
    ...
    {
      title: 'Logging: info, warn, error',
      content: `Workers can send messages visible in the UI:
  ...
  q_method_call: [
    {
      ...
      content: `In Python, a **method** is a function that belongs to an object. You call it with dot notation:
...
Each method call tells your worker to **do something**. Methods are how you interact with the game world.`,
    ...
    {
      title: 'Write Your First Worker',
      content: `Open \`workspace/workers/miner.py\` (or \`miner.js\`) and write:
...
An \`Edge\` is a single connection between two adjacent nodes. At deploy time you click one edge on the map. \`self.move(edge)\` walks back and forth along it.`,
...

    {
      ...
      content: `1. Click the **Hub** node → **Deploy Worker**
  ...
  q_dot_notation: [
    {
      ...
      content: `Objects have **attributes** you can read with dot notation:
...
Dot notation lets you **inspect** the world before acting on it.`,
...

    {
      ...
      content: `Look at the map — some nodes are **locked** (greyed out). Click on a locked node to see:
  ...
  q_conditions: [
    {
      ...
      content: `Every data mine has a chance of producing **bad data** — corrupted fragments mixed in with good data.
...
You need to learn \`if\` statements to **filter out bad data** before depositing.`,
...

    {
      ...
      content: `An \`if\` statement lets your code make decisions:
...
- \`self.discard()\` — throw away the held item without depositing`,
    ...
    {
      title: 'Smart Miner with Filtering',
      content: `Here's a miner that filters bad data:
...
These are used inside \`if\` statements to make numeric decisions.`,
...

    {
      ...
      content: `Some nodes get **infected** — they turn red and spread infection to neighbors.
...
Unlike \`for\` loops (which iterate a known collection), \`while\` loops handle **unknown** amounts of work. You don't know in advance how many times you'll loop.`,
    ...
    {
      title: 'Filtering Bad Data',
      content: `Some resource nodes produce **bad_data** drops. You need to filter them out:
...
**Goal:** Deposit **2 kB data total**. The while loop helps you filter efficiently.`,
  ...
  q_for_loop: [
    {
      title: 'Routes & For Loops',
      content: `So far you've used \`Edge\` — a single connection between two adjacent nodes. But what if the mine is **far away**, separated by relay nodes?
...
\`\`\``,
    ...
    {
      title: 'Build a Long-Range Miner',
      content: `Create \`workspace/workers/long_range_miner.py\`:
...
Register it in \`main.py\`, then deploy with a route that reaches a distant mine (e.g. Data Mine Alpha or Beta).`,
    ...
    {
      title: 'Deploy & Test',
      content: `1. Register \`LongRangeMiner\` in \`main.py\`
...
**Goal:** Mine **20 times** using routes to reach distant, higher-yield mines.`,
  ...
  q_cluster_mining: [
    {
      title: 'The Data Mine Cluster',
      content: `South of the Hub, there's a **Data Mine Cluster** — a relay node surrounded by small resource nodes.
...
You'll need an \`AdvancedSensor\` — a gadget that scans adjacent edges and tells you the **type** of each connected node.`,
    ...
    {
      title: 'Build a Cluster Miner',
      content: `Create \`workspace/workers/cluster_miner.py\`:
  ...
  q_try_except: [
    {
      ...
      content: `Things go wrong. Nodes deplete, inventory fills up, moves fail. \`try/except\` catches errors so your worker doesn't crash:
...
Without error handling, one unexpected failure kills your entire worker process.`,
...

    {
      ...
      content: `A resilient worker wraps risky operations:
  ...
  q_event_loop: [
    {
      ...
      content: `Your worker's \`on_loop()\` method is called repeatedly -- it's already a loop!
...
Each call to \`on_loop()\` is one iteration. Your miner mines, collects, deposits, and then the loop repeats.`,
    },
    {
      ...
      content: `Let your miner run and watch the logs. Each cycle is one loop iteration:
  ...
  q_batch_processing: [
    {
      ...
      content: `A for loop processes items one at a time. Your miners do the same:
...
Craft **3 items** using the Inventory panel (press \`E\`). You can craft pickaxes, shields, and beacons.`,
  ...
  q_error_handling: [
    {
      title: 'Try / Except',
      content: `Workers can crash. When they do, the error is logged and you can redeploy.
  ...
  q_graph_theory: [
    {
      ...
      content: `A graph \`G = (V, E)\` consists of vertices (nodes) and edges (connections).
...
Different topologies affect worker routing. Hub-and-spoke, mesh, tree -- each has tradeoffs.`,
  ...
  q_routing: [
    {
      title: 'Shortest Path',
      content: `Routers use BFS/Dijkstra to find shortest paths:
  ...
  q_relay_network: [
    {
      ...
      content: `Relay nodes provide alternative paths. If one path is blocked (infection), workers can route around it.
  ...
  q_dns_lookup: [
    {
      ...
      content: `DNS resolves names to addresses. Workers use \`scan()\` to discover what is around:
  ...
  q_firewall: [
    {
      ...
      content: `Firewalls use rules to filter traffic. Chips add defensive layers to your nodes.
  ...
  q_antivirus: [
    {
      ...
      content: `Antivirus software scans for known patterns. Your Guardian does the same:
  ...
  q_redundancy: [
    {
      ...
      content: `Redundant systems survive failures:
...
Deploy **8 workers** total to build a fault-tolerant workforce.`,
  ...
  q_profiling: [
    {
      title: 'Measure First',
      content: `Never optimize without measuring. Node upgrades show measurable improvement.
  ...
  q_memory_mgmt: [
    {
      ...
      content: `Efficient programs manage memory carefully:
  ...
  q_full_stack: [
    {
      ...
      content: `A full stack developer knows it all: frontend, backend, databases, deployment.
