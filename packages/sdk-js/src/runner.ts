...
async function main(): Promise<void> {
  ...
  const apiKey = process.env['NETCRAWL_API_KEY'] ?? '';
  const generation = Number(process.env['NETCRAWL_GENERATION'] ?? '0');
  const executionToken = process.env['NETCRAWL_EXECUTION_TOKEN'] ?? '';
  ...
  const worker = new (WorkerCls as new (
    workerId: string,
    apiUrl: string,
  ...
  ) => WorkerClass)(workerId, apiUrl, injectedFields, apiKey, generation, executionToken);
