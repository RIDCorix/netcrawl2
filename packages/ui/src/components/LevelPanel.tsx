...
function rewardIcon(kind: string) {
  switch (kind) {
    case 'flop_bonus':
      return <Zap size={12} style={{ color: MILESTONE_COLORS.flop_bonus }} />;
    case 'max_workers_bonus':
      return <Star size={12} style={{ color: MILESTONE_COLORS.max_workers_bonus }} />;
    case 'passive':
      return <Shield size={12} style={{ color: MILESTONE_COLORS.passive }} />;
    case 'recipe_unlock':
      return <Pickaxe size={12} style={{ color: MILESTONE_COLORS.recipe_unlock }} />;
    case 'items':
      return <Gift size={12} style={{ color: MILESTONE_COLORS.items }} />;
    default:
...
function rewardLabel(reward: any, t: (key: string) => string): string {
  switch (reward.kind) {
    case 'flop_bonus':
      return `+${reward.value} FLOP`;
    case 'max_workers_bonus':
      return t('ui.worker_slot').replace('{value}', reward.value);
    case 'passive':
      return reward.description;
    case 'recipe_unlock':
      return `Unlock: ${reward.name}`;
    case 'items':
      return reward.items.map((i: any) => `${i.count}x ${i.itemType}`).join(', ');
    default:
...
export function LevelPanel() {
  ...
  const { level, xp, xpToNext, totalXp, title, titleZh, maxLevel, maxWorkersBonus, flopBonus, milestones } =
    levelSummary;
          ...
          >
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div>
                  ...
                  <div
                    style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginTop: 2 }}
                  >
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
                    {t('ui.flop_workers')
                      .replace('{flop}', String(flopBonus))
                      .replace('{workers}', String(maxWorkersBonus))}
                  </div>
                </div>
              </div>
                ...
                >
                  {isMaxLevel
                    ...
                    : t('ui.xp_progress')
                        ...
                        .replace('{next}', xpToNext.toLocaleString())}
                </div>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '12px 20px' }}>
              ...
              {milestones.map(m => {
                ...
                const isCurrent =
                  level < m.level && milestones.filter(mm => mm.level <= level).length === milestones.indexOf(m);
