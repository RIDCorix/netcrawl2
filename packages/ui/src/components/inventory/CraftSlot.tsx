    ...
    >
      <button
        ...
        style={{
          ...
          animation:
            isPending && !revealing
              ? 'unlock-flash 1.5s ease-in-out infinite'
              : revealing
                ? 'unlock-burst 0.5s ease-out forwards'
                : 'none',
        }}
      >
        <Icon
        ...
        {!locked && (
          ...
          >
            {recipe.cost.data !== undefined && (
              <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--data-color)' }} />
            )}
            {recipe.cost.rp !== undefined && (
              <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--rp-color)' }} />
            )}
            {recipe.cost.credits !== undefined && (
              <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--credits-color)' }} />
            )}
          </div>
        )}
      </button>
          ...
          >
            {isPending && !revealing ? (
              ...
              <>
                ...
                <div
                  style={{ fontSize: 8, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: 6 }}
                >
                  {displayDesc}
                </div>
                <div
                ...
                >
                  {Object.entries(recipe.cost).map(([key, needed]) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: cfg.color, fontWeight: 600 }}
                        >
                          {cfg.label}
                        </span>
                        <span
...
export function CraftFamilyColumn({
  family,
  recipes,
  dimmer,
  onCraft,
  ...
  const familyRecipes = family.recipeIds.map(id => recipes.find(r => r.id === id)).filter(Boolean) as Recipe[];
