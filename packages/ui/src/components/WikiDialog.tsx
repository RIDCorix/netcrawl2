...
function useProgress(): PlayerProgress {
  ...
  return useMemo(
    () => ({ level, questsCompleted, activeLayer, unlockedRecipes }),
    [level, questsCompleted, activeLayer, unlockedRecipes],
  );
}
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        gap: 10,
        padding: '10px 12px',
        background: active ? `color-mix(in srgb, ${category.color} 18%, var(--bg-primary))` : 'transparent',
        border: `1px solid ${active ? `color-mix(in srgb, ${category.color} 50%, transparent)` : 'var(--border)'}`,
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        gap: 8,
        padding: '8px 10px',
        background: selected ? `color-mix(in srgb, ${color} 15%, var(--bg-primary))` : 'transparent',
        border: selected ? `1px solid color-mix(in srgb, ${color} 45%, transparent)` : '1px solid transparent',
...
export function WikiDialog() {
  ...
  const unlockedFn = useMemo(() => (entry: WikiEntry) => isEntryUnlocked(entry.unlock, progress), [progress]);
        ...
        >
          <motion.div
            ...
            style={{
              ...
              boxShadow:
                '0 20px 80px rgba(0,0,0,0.6), inset 0 0 0 1px color-mix(in srgb, var(--accent) 8%, transparent)',
              display: 'flex',
              flexDirection: 'column',
              overflow: 'hidden',
            }}
          >
            ...
            >
              ...
              <div style={{ position: 'relative' }}>
                {selectedEntryId && !selectedLookup ? (
                  <div role="alert" style={{ padding: 32, fontFamily: 'var(--font-mono)' }}>
                    <h2 style={{ color: 'var(--danger)', fontSize: 16 }}>{t('wiki.invalid_entry.title')}</h2>
                    <p style={{ color: 'var(--text-secondary)', lineHeight: 1.7 }}>{t('wiki.invalid_entry.body')}</p>
                    <button
                      onClick={() => handleSelectCategory(WIKI[0].id)}
                      style={{
                        padding: '8px 12px',
