...
export function EquipmentStep({
  ...
  const equipItem = (itemType: string) => {
    const slot = itemSlots.find(s => !equipped[s.name] && (SLOT_ACCEPTS[s.itemType] || []).includes(itemType));
    if (!slot) return;
    setEquippedPerUnit(prev => {
      const next = [...prev];
      next[currentUnitIdx] = { ...(next[currentUnitIdx] || {}), [slot.name]: itemType };
      return next;
    });
  };
  const missingSlots = equippedPerUnit.flatMap((unit, unitIdx) =>
    classItemSlots
      .filter(slot => !unit?.[slot.name])
      .map(slot =>
        unitCount > 1
          ? t('deploy.missing_slot_unit')
              .replace('{unit}', String(unitIdx + 1))
              .replace('{slot}', slot.name)
          : t('deploy.missing_slot').replace('{slot}', slot.name),
      ),
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <HardwareRow
          ...
          onDecrement={() =>
            setCpuPerUnit(prev => {
              const n = [...prev];
              n[currentUnitIdx] = Math.max(0, (n[currentUnitIdx] || 0) - 1);
              return n;
            })
          }
          onIncrement={() =>
            setCpuPerUnit(prev => {
              const n = [...prev];
              n[currentUnitIdx] = (n[currentUnitIdx] || 0) + 1;
              return n;
            })
          }
        />
      ...
      {classItemSlots.length > 0 && (
        <>
          <div
            style={{ height: 1, background: 'linear-gradient(90deg, transparent, var(--border-bright), transparent)' }}
          />
          <div
            style={{
              fontSize: 11,
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {itemSlots.map(slot => (
              <EquipSlot
                key={slot.name}
                slotName={slot.name}
                acceptType={slot.itemType}
                equipped={
                  equipped[slot.name] ? playerInventory.find(i => i.itemType === equipped[slot.name]) || null : null
                }
                onEquip={t => {
                  setEquippedPerUnit(prev => {
                    const next = [...prev];
                ...
                removeLabel={t('deploy.remove_item').replace('{item}', slot.name)}
                emptyLabel={t('deploy.empty_slot').replace('{slot}', slot.name)}
              />
            ))}
          </div>
          {!allSlotsFilled && (
            <div
              id="deploy-equipment-requirement"
              role="status"
              style={{ fontSize: 10, color: 'var(--warning)', fontFamily: 'var(--font-mono)' }}
            >
              {t('deploy.missing_requirements').replace('{requirements}', missingSlots.join(', '))}
            </div>
          )}
        </>
      )}
      ...
      {classItemSlots.length > 0 && (
        <>
          ...
          {availableInventory.filter(
            i => !['cpu_basic', 'cpu_advanced', 'ram_basic', 'ram_advanced'].includes(i.itemType),
          ).length === 0 ? (
            <div
              style={{
                fontSize: 11,
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {availableInventory
                .filter(i => !['cpu_basic', 'cpu_advanced', 'ram_basic', 'ram_advanced'].includes(i.itemType))
                .map(item => (
                  <InvItem
                    key={item.itemType}
                    item={item}
                    disabled={
                      !itemSlots.some(
                        s => !equipped[s.name] && (SLOT_ACCEPTS[s.itemType] || []).includes(item.itemType),
                      )
                    }
                    onSelect={() => equipItem(item.itemType)}
                    selectLabel={t('deploy.equip_item').replace('{item}', item.itemType)}
                  />
                ))}
            </div>
          )}
...
function HardwareRow({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <button onClick={onDecrement} disabled={value <= 0} style={btnStyle(value <= 0)}>
        -
      </button>
      <span
        style={{
      ...
      <button onClick={onIncrement} disabled={value >= max} style={btnStyle(value >= max)}>
        +
      </button>
