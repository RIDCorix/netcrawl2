...
function QuestNode({ data }: any) {
  ...
  const bg = status === 'claimed' ? `${color}15` : status === 'completed' ? `${color}10` : 'var(--bg-glass-heavy)';
    ...
    >
          ...
          <div key={obj.id} style={{ marginBottom: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span
                style={{
                  fontSize: 10,
                  color: obj.met ? 'var(--success)' : 'var(--text-secondary)',
                  fontFamily: 'var(--font-mono)',
                }}
              >
              ...
              <span
                style={{
                  ...
                  fontWeight: 700,
                  fontFamily: 'var(--font-mono)',
                  color: obj.met ? 'var(--success)' : 'var(--text-muted)',
                }}
              >
            ...
            <div
              style={{ height: 3, borderRadius: 2, background: 'var(--bg-primary)', marginTop: 3, overflow: 'hidden' }}
            >
              <div
                style={{
                  height: '100%',
                  width: `${Math.min(100, (obj.current / obj.target) * 100)}%`,
                  background: obj.met ? 'var(--success)' : color,
                  borderRadius: 2,
            ...
            >
              {r.kind === 'resources' &&
                Object.entries(r.resources)
                  .map(([k, v]) => `${formatResource(k, v as number)} ${k}`)
                  .join(', ')}
      ...
      {msg && (
        <div
          style={{
            fontSize: 10,
            color: msg.includes('claimed') ? 'var(--success)' : 'var(--danger)',
            fontFamily: 'var(--font-mono)',
            textAlign: 'center',
          }}
        >
...
export function QuestTree() {
  ...
  const getChapterProgress = (ch: number) => {
    if (!questData) return { requiredClaimed: 0, requiredTotal: 0, optionalClaimed: 0, optionalTotal: 0 };
    const cq = questData.quests.filter((q: any) => q.chapter === ch);
    const required = cq.filter((q: any) => q.mainline);
    const optional = cq.filter((q: any) => !q.mainline);
    return {
      requiredClaimed: required.filter((q: any) => q.status === 'claimed').length,
      requiredTotal: required.length,
      optionalClaimed: optional.filter((q: any) => q.status === 'claimed').length,
      ...
      {questsOpen && (
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
              {chapters.map(ch => {
                ...
                const { requiredClaimed, requiredTotal, optionalClaimed, optionalTotal } = getChapterProgress(ch);
                const isActive = ch === activeChapter;
                const allClaimed = requiredClaimed === requiredTotal && requiredTotal > 0;
                return (
                  <button
                    key={ch}
                  ...
                  >
                      ...
                      >
                        <div
                          style={{
                            height: '100%',
                            borderRadius: 2,
                            width: requiredTotal > 0 ? `${(requiredClaimed / requiredTotal) * 100}%` : '0%',
                            background: color,
                    ...
                    >
                      <span>
                        {t('quest.required_short')} {requiredClaimed}/{requiredTotal}
                      </span>
                      {optionalTotal > 0 && (
                        <span style={{ display: 'block', marginTop: 2 }}>
                          {t('quest.optional_short')} {optionalClaimed}/{optionalTotal}
                        </span>
                      )}
                    </span>
                  </button>
          ...
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            >
              {(() => {
                const Icon = CHAPTER_ICONS[activeChapter] || BookOpen;
                return <Icon size={16} style={{ color: CHAPTER_COLORS[activeChapter] || '#666' }} />;
              })()}
              <span
                style={{
                  fontSize: 13,
                  fontWeight: 800,
                  fontFamily: 'var(--font-mono)',
                  color: CHAPTER_COLORS[activeChapter] || '#666',
                  letterSpacing: '0.08em',
                }}
              >
            ...
            >
              <Background
                ...
                gap={24}
                size={1}
                color={`color-mix(in srgb, ${CHAPTER_COLORS[activeChapter] || '#666'} 5%, transparent)`}
              />
            </ReactFlow>
          </div>
          ...
          <AnimatePresence>
            {selectedQuest && <QuestGuideDialog quest={selectedQuest} onClose={() => selectQuest(null)} />}
          </AnimatePresence>
        </motion.div>
      )}
    </AnimatePresence>
