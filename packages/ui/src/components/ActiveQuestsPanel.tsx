...
export function ActiveQuestsPanel() {
    ...
    axios
      ...
      .then(r => {
        ...
        const active = all.filter(
          (q: any) => q.id !== 'q_ch1_challenge' && (q.status === 'available' || q.status === 'completed'),
        );
        setQuests(active);
      })
      .catch(() => {});
  ...
  return (
    <>
            ...
            >
              <div style={{ padding: '6px 4px', overflowY: 'auto', maxHeight: 'calc(50vh - 130px)' }}>
                ...
                {displayQuests.map(q => {
                  ...
                  const progress =
                    q.objectives.length > 0
                      ? q.objectives.reduce((sum: number, o: any) => sum + Math.min(1, o.current / o.target), 0) /
                        q.objectives.length
                      : 0;
                  ...
                  return (
                    <motion.div
                      ...
                      animate={
                        justClaimed ? { height: 0, opacity: 0, marginBottom: 0 } : { height: 'auto', opacity: 1 }
                      }
                      transition={
                        justClaimed
                          ? {
                              height: { duration: 0.3, delay: 0.7, ease: 'easeInOut' },
                              opacity: { duration: 0.15, delay: 0.85 },
                              marginBottom: { duration: 0.3, delay: 0.7 },
                            }
                          : { duration: 0.15 }
                      }
                    ...
                    >
                      <div
                        style={{
                          ...
                          border: 'none',
                          borderRadius: 'var(--radius-sm)',
                          cursor: justClaimed ? 'default' : 'pointer',
                          textAlign: 'left',
                                ...
                                >
                                  ...
                                  >
                                    <span
                                      style={{
                                        fontSize: 9,
                                        color: obj.met ? '#4ade80' : 'var(--text-muted)',
                                        fontFamily: 'var(--font-mono)',
                                        overflow: 'hidden',
                                        textOverflow: 'ellipsis',
                                        whiteSpace: 'nowrap',
                ...
                {claimError && (
                  <div
                    role="alert"
                    style={{ padding: '4px 8px', color: 'var(--danger)', fontSize: 9, fontFamily: 'var(--font-mono)' }}
                  >
                    {claimError}
                  </div>
                )}
      ...
      {/* Guide dialog */}
      <AnimatePresence>
        {selectedQuest && <QuestGuideDialog quest={selectedQuest} onClose={() => setSelectedQuest(null)} />}
      </AnimatePresence>
    </>
  );
}
