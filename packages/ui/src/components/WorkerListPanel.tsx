...
import { Users, ChevronDown, ChevronRight, X, PauseCircle, Square, Loader, Play } from 'lucide-react';
...
function ClassGroup({
  ...
  const running = workers.filter(
    w => w.status === 'running' || w.status === 'moving' || w.status === 'harvesting',
  ).length;
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      ...
      {/* Workers */}
      <AnimatePresence>
        {expanded && workers.map(w => <WorkerRow key={w.id} worker={w} onSuspend={onSuspend} onDismiss={onDismiss} />)}
      </AnimatePresence>
    </div>
  );
}
        ...
        <span style={{ width: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {config.spin ? (
            ...
            <svg width={8} height={8} viewBox="0 0 8 8">
              ...
              {config.dot === 'x' && (
                <>
                  <line x1={1.5} y1={1.5} x2={6.5} y2={6.5} stroke={config.color} strokeWidth={1.5} />
                  <line x1={6.5} y1={1.5} x2={1.5} y2={6.5} stroke={config.color} strokeWidth={1.5} />
                </>
              )}
            </svg>
          )}
            ...
            >
              {logs.length === 0 ? (
                ...
                logs.map((log, i) => (
                  <div
                    key={i}
                    style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: 1 }}
                  >
...
export function WorkerListPanel() {
  ...
  const handleDeployAllSuspended = async () => {
    for (const w of suspendedWorkers) {
      try {
        await axios.post('/api/worker/reset', { workerId: w.id });
      } catch {}
    }
  };
