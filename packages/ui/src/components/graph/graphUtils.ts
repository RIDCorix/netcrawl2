...
export function toRFNodes(
  ...
  routePath: string[] = [],
  unlockableNodeIds: Set<string> = new Set(),
): Node[] {
  return gameNodes.map(n => ({
    ...
    data: {
      ...n.data,
      label: tn(n.data.label),
      selected: n.id === selectedId,
      showWorkerDots,
      edgeStyle,
      fadeIn: fadeInIds.has(n.id),
      unlockable: unlockableNodeIds.has(n.id),
      routeIndices: routePath.reduce<number[]>((acc, id, i) => {
        if (id === n.id) acc.push(i);
        return acc;
      }, []),
    },
    selected: n.id === selectedId,
