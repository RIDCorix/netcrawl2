        ...
        {visibleWorkers.map((w: any, wi: number) => {
          ...
          return (
            ...
            >
              ...
              {showErrorBubble && (
                <WorkerBubble type="error" worker={w} workerIndex={wi} totalWorkers={workers.length} color="#ef4444" />
              )}
              {showInfoBubble && (
                <WorkerBubble
                  type="info"
                  ...
                  color={w.lastLog.level === 'error' ? '#ef4444' : w.lastLog.level === 'warn' ? '#f59e0b' : c}
                />
              )}
            </motion.div>
          );
...
function WorkerBubble({
  ...
  return (
    <div
      key={isError ? undefined : `b-${worker.lastLog.ts}`}
      style={{
        position: 'absolute',
        left: 2,
        bottom: 6,
